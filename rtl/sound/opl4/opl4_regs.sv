...
//
// Port map (offset within the chip select; Z80 I/O 0x08+offset on Psikyo):
...
module opl4_regs (
	...
	// Channel registers 0x20-0xF7 as they stood at the start of the sample
	// pass, as ymfm's prepare() reads them. The engine visits channels one at
	// a time across the pass; reading live, a channel whose slot falls between
	// two driver writes latches the intermediate value. The drivers park the
	// level register at TL 127 with level-direct set before writing the real
	// level, and a latched park ramps the note in from silence over ~80 ms.
