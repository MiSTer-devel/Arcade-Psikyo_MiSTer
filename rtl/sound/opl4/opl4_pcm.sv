...
module opl4_pcm (
	...
	// The engine advances only on cen (clk/2); Psikyo.sdc states its
	...
	// Full-rate latches. Named fr_* because Psikyo.sdc excludes them
	...
	always_ff @(posedge clk or posedge reset) begin
		if (reset) begin
			...
			// opl4.sv's arbiter and opl4_regs expect; Psikyo.sdc excludes
			...
			case (state)
				...
				S_RD1: begin
					...
					// ymfm does: position, envelope, level interpolation and LFO
					// all advance. Do not skip released silent channels: one
					// re-keyed without a wave load then starts from a stale
					// level and envelope. Silent channels still skip the fetch.
